...
data class FollowersGetResponseData (
    ...
    internal fun parse(response: Response): FollowersGetResponseData? {
         response.apply {
            if(response.code != 200) {
                return throw AnnictError(response.message)
            }
             return body?.string()?.let { Json { isLenient = true }.decodeFromString<FollowersGetResponseData>(it) }
        }
    }
}
...
class FollowersService(val client: AnnictClient) {
    ...
    internal fun get(query: FollowersGetRequestQuery): FollowersGetResponseData? {
        this.client.apply {
